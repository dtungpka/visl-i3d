...
class DSL:
    ...
    def get_generator(self,filtered_list=None,randomize=True,output='rgb'):
        ...
        return Generator(filtered_list,self.height,self.width,self.n_frames,self.batch_size,self.classes,output=output,cache_folder=self.cache_folder)
        
optical_flow = cv2.optflow.createOptFlow_DualTVL1()
class Generator(torch.utils.data.IterableDataset ):
    def __init__(self,data_paths,height,width,n_frames,batch_size,classes,output,cache_folder) -> None:
    ...
    def get_data(self,start,end):
        ...
        should_cache = self.cache_folder is not None
        #if cache folder is not None, create cache folder
        if should_cache:
        ...
        for i in range(start,end):
            ...
            if should_cache:
                #base name of depth_batch[i] and remove the extension
                cache_file = os.path.join(self.cache_folder,os.path.basename(depth_batch[i]).replace('.npy','')+f"_{self.output}.npy")
            ...
            if X is None:
                ...
                if self.output == 'flow':
                    ...
                    combined = np.array([cv2.resize(read_depth[j], (self.width, self.height)) for j in range(read_depth.shape[0])])
                    prev = combined[0]
                    X = np.zeros((n_frames, combined.shape[1], combined.shape[2], 2), dtype=np.float32)
                    for _i in range(1, n_frames):
                        X[_i] = optical_flow.calc(prev,combined[_i], None)
                        prev = combined[_i]
